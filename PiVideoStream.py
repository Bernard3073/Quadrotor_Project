...
class PiVideoStream:
	...
	def readfollow(self):
                ...
                lower_red_0 = np.array([0, 0, 0]) 
                upper_red_0 = np.array([179, 255, 10])
                
                lower_red_1 = np.array([0, 0, 0]) 
                upper_red_1 = np.array([179, 255, 63])
                ...
                if (newred1lConfidence > 30) and (newRed2Confidence > 30):
                ...
                g_sensitivity = -4
                g =32
                lower_green_0 = np.array([60-g_sensitivity,56,0]) 
                upper_green_0 = np.array([60+g,255,255])

                lower_green_1 = np.array([60-g_sensitivity,65,26]) 
                upper_green_1 = np.array([60+g,255,255])
                # Prepare the masks for the lower roi 
                maskr_33 = preparemask (roihsv3, lower_green_0 , upper_green_0)
                maskr_44 = preparemask (roihsv3, lower_green_1 , upper_green_1)
                ...
                if len(cnts_green) > 0:
                    ...
                    cx_green = cx_green+110
                    cy_green = cy_green+70
                    
                if cx_green > 135 and cx_green < 185 and cy_green >95 and cy_green < 145:
                    circle=1
                else:
                    circle=0
                '''
                if mask is not None:
                    cnts_blue1 = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL,
                                cv2.CHAIN_APPROX_SIMPLE)[-2]
                    centerb1 = None
                    ...
                    if len(cnts_blue1) > 0:
                        ...
                        bluey = cy_blue1'''
                moving_ratio = 0
                if cx_red1 <= 0 and cx_red2 <= 0:
                    rlcontrol = 0
                else:
                    if cx_red1 > 190 and cx_red2 > 190:
                    ...
                    elif cx_red1 < 130 and cx_red2 < 130:
		...
		return (bearing, red1Good, offset, red2Good, circle, rlcontrol, moving_ratio)
	...
	def readlost(self):
	        ...
	        #ewarped = cv2.warpPerspective(frame, M, (600, 268))
                ...
                '''
                    cnts_blue2 = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL,
                                cv2.CHAIN_APPROX_SIMPLE)[-2]
                    centerb2 = None
		...
		return (bearing, redl1Good, offset, redl2Good, circle, rlcontrol, moving_ratio)
